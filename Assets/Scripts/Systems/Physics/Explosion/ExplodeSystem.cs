                    ...
                    if(!alreadyHitRigidbodies.GetSubArray(0, alreadyHitRigidbodiesIndex).Contains(hit.RigidBodyIndex))
                    {
                        if(PhysicsVelocityLookup.HasComponent(hit.Entity))
                        {
                            ...
                            hitVelocity.ValueRW.ApplyExplosionForce(hitMass.ValueRO, hitCollider.ValueRO, hitBody.WorldFromBody.pos, 
                                ...
                                Explosion.Config.Radius, FixedDeltaTime, up, collisionFilter, Explosion.Config.UpwardsModifier, ForceMode.Impulse);

                            var dragOfHitEntity = PhysicsDampingLookup.GetRefRWOptional(hit.Entity, false);
                            var dragBeforeExplosion = float.NaN;
                            if(dragOfHitEntity.IsValid)
                            {
                                dragBeforeExplosion = dragOfHitEntity.ValueRO.Linear;
                            ...
                            EntityCommandBuffer.AddComponent(hitBody.Entity, new HitByExplosion(dragBeforeExplosion));
                        }
                    }
                }
